...
?>
	...
	<ul id="board_list">
		<li class="title">
			...
			<span class="col5">점수</span>
			<span class="col6">게임</span>
		</li>
		<form method="post" action="admin_board_delete.php">
<?php
	$sql = "select *, review.point as review_point, game.name as game_name from review join game on review.game_id = game.game_id join members on review.user_id = members.num order by review.point";
	$result = mysqli_query($con, $sql);
	$total_record = mysqli_num_rows($result); // 전체 글의 수
   ...
   while ($row = mysqli_fetch_array($result))
   {
	  ...
	  $name        = $row["name"];
	  $subject     = $row["content"];
	  $score       = $row["review_point"];
      $regist_day  = $row["game_name"];
?>
		<li>
			<span class="col1"><input type="checkbox" name="item[]" value="<?=$num?>"></span>
			<span class="col2"><?=$number?></span>
...
?>
				<button type="submit">선택된 리뷰 삭제</button>
			</form><!-- 리뷰 추가 부분 -->
<?php
   mysqli_close($con);
?>
